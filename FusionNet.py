...
def unit_test():
    import numpy as np
    x = torch.tensor(np.random.rand(2,1,271,271).astype(np.float32))
    y = torch.tensor(np.random.rand(2,1,271,271).astype(np.float32))
    model = FusionNet(output=1)
    z = model(x,y)
    print('output shape:', z.shape)
    assert z.shape == (2,1,271,271), 'output shape (2,3,271,271) is expected!'
    print('test ok!')
