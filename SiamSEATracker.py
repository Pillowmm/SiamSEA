...
from .rgbt_network import SiamSEANet
...
class SiamSEATracker:
    ...
    def init(self, rgb_frame, t_frame, bbox, bbox_r, bbox_t):
        ...
        fusion_exemplar_img = torch.mean(t_exemplar_img, dim=1, keepdim=True)
        self.model.track_init(rgb_exemplar_img.cuda(), t_exemplar_img.cuda(), fusion_exemplar_img.cuda())
    ...
    def update(self, rgb_frame, t_frame):
        ...
        fusion_instance_img = torch.mean(t_instance_img, dim=1, keepdim=True)
        pred_score_1, pred_score_2, pred_regression = self.model.track(rgb_instance_img.cuda(), t_instance_img.cuda(), fusion_instance_img.cuda())
    ...
    def track(self, rgb_img_files, t_img_files, groundtruth, bbox_rgb, bbox_t, visualize=False):
